...
if [ ! -z ${OPCACHE_MEM_SIZE} ]; then
    STEP='Configuring APC shim size value'
    echo "[....] "${STEP}
    echo 'apc.shim_size=${OPCACHE_MEM_SIZE}' >> /etc/php5/conf.d/20-apc.ini
    echo "[ OK ] "${STEP}
fi
...
if [ ! -z ${PHP_CLI_MEMORY_LIMIT} ]; then
    STEP='Configuring PHP CLI memory limit'
    echo "[....] "${STEP}
    sed -i 's#memory_limit = .*#memory_limit = ${PHP_CLI_MEMORY_LIMIT}#g' /etc/php5/cli/php.ini
    echo "[ OK ] "${STEP}
fi
